    ...
    def validate(cls, data: Dict[str, Any]) -> List[ValidationError]:
        ...
        if ac_data and isinstance(ac_data, dict):
            ...
            if roles is not None and not isinstance(roles, dict):
                errors.append(ValidationError("access_control.roles", "Must be a dictionary mapping role names to IDs", roles))
            channels = ac_data.get("channels")
            if channels is not None and not isinstance(channels, dict):
                errors.append(ValidationError("access_control.channels", "Must be a dictionary mapping channel names to IDs", channels))

        # 3. Validate Bots
        raw_bots = data.get("bots", {})
        if not isinstance(raw_bots, dict):
            errors.append(ValidationError("bots", "Must be a dictionary mapping bot IDs to bot configurations", raw_bots))
        else:
            ...
            for bot_id, bot_cfg in raw_bots.items():
                ...
                if not name or not isinstance(name, str) or not name.strip():
                    errors.append(ValidationError(f"{prefix}.name", "Bot name is required and must be a non-empty string", name))
                ...
                if is_windows and (not path or not str(path).strip()):
                    path = bot_cfg.get("path")

                if path is None or (not isinstance(path, str)):
                    errors.append(ValidationError(f"{prefix}.path", "Bot working directory path is required (path or path_win)", path))

                # Command validation (cross-platform)
                cmd = bot_cfg.get("cmd_win") if is_windows and "cmd_win" in bot_cfg else bot_cfg.get("cmd")
                if is_windows and (not cmd or not str(cmd).strip()):
                    cmd = bot_cfg.get("cmd")

                if cmd is None or (not isinstance(cmd, str)):
                    errors.append(ValidationError(f"{prefix}.cmd", "Bot startup command is required (cmd or cmd_win)", cmd))

                # db_files validation
                db_files = bot_cfg.get("db_files")
                if db_files is not None and not isinstance(db_files, list):
                    errors.append(ValidationError(f"{prefix}.db_files", "Must be a list of database filenames", db_files))
