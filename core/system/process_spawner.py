...
class ProcessSpawner:
    ...
    def __init__(self, stop_timeout: float = 5.0, restart_wait: float = 1.0, log_rotator: ILogRotator | None = None):
    ...
    @staticmethod
    def validate_command(cmd: str) -> tuple[bool, str | None]:
    ...
    def spawn(self, bot_config: BotConfig, env: dict) -> int | None:
    ...
    def find_all_processes_in_path(self, bot_path: str) -> list[int]:
        ...
        Strictly excludes the current manager process (os.getpid()) to prevent suicide bugs.
        ...
        for proc in psutil.process_iter(["pid", "name"]):
            try:
                ...
                if is_subpath_or_equal(cwd, bot_path) and proc.pid != current_pid:
    ...
    async def kill_rogue_processes(self, bot_path: str) -> None:
        ...
        # Safety: Never perform rogue process cleanup on current working directory (.) or manager root
        if not bot_path or os.path.abspath(bot_path) == os.path.abspath("."):
            log.warning(f"[ProcessSpawner] Skipped rogue process cleanup for manager root path '{bot_path}'")
            return
    ...
    def check_systemd_privileges() -> tuple[bool, str]:
        ...
        try:
            res = subprocess.run(
                ["sudo", "-n", "systemctl", "--version"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
            if res.returncode == 0:
    ...
    def get_systemd_pid(self, service_name: str) -> int | None:
        if os.name != "posix":
            return None
        try:
            result = subprocess.run(
    ...
    async def get_systemd_pid_async(self, service_name: str, retries: int = 3) -> int | None:
        async def _fetch():
