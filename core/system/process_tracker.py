...
class ProcessTracker:
    ...
    RUNTIME_EXECUTABLES = ("python", "java", "node", "pypy", "uv", "uvicorn", "dotnet", "ruby", "deno", "bun")
    ...
    def discover_processes(self, bots: dict[str, BotConfig]) -> int:
        ...
        Supports both script-based bots and single-word compiled binaries (e.g. Go, Rust, C++).
        """
        found_count = 0
        current_pid = os.getpid()

        # Collect possible executable basenames from configured bots
        target_executables = set(self.RUNTIME_EXECUTABLES)
        for b in bots.values():
            if b.cmd:
                first_part = b.cmd.split()[0].lower()
                exe_name = os.path.basename(first_part).replace(".exe", "")
                if exe_name:
                    target_executables.add(exe_name)
        ...
        for proc in psutil.process_iter(["pid", "name"]):
            try:
                ...
                if not any(name.startswith(exe) or exe.startswith(name) for exe in target_executables):
                    continue

                cmdline = proc.cmdline()
                cwd = proc.cwd()
                if not cmdline or not cwd or len(cmdline) < 1:
                    continue
                ...
                for bot_id, bot_cfg in bots.items():
                    ...
                    target_bin = os.path.basename(target_parts[0]).replace(".exe", "")

                    path_match = is_subpath_or_equal(cwd, bot_cfg.path)
                    cmd_match = (
                        (target_args in cmd_str)
                        or (target_bin and target_bin in name)
                        or (target_bin and target_bin in cmd_str)
                    )
    ...
    def get_stats(
        self, bot_id: str, bot_cfg: BotConfig | None = None, all_bots: dict[str, BotConfig] | None = None
    ) -> dict[str, Any] | None:
        """Returns CPU %, RAM MB, Uptime seconds, PID, and disk I/O for a tracked bot."""
        process = self.managed_processes.get(bot_id)

        # Proactive discovery if process is missing or dead
        if (not process or not self.is_running(bot_id)) and all_bots:
        ...
        try:
            ...
            return {"cpu": cpu, "ram_mb": ram_mb, "uptime_sec": uptime_sec, "pid": process.pid, "disk_mb": disk_mb}
    ...
    def fetch_unexpected_stops(
        self, bots: dict[str, BotConfig], spawner: Optional["IProcessSpawner"] = None
        ...
        for bot_id, bot_cfg in bots.items():
            ...
            if bot_id in self.manual_stop and (not process or not self.is_running(bot_id)):
