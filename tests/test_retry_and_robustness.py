...
def test_retry_sync_succeeds_on_transient_failure():
    ...
    res = retry_sync(
        flaky_func, max_retries=3, initial_delay=0.01, backoff_factor=1.0, jitter=False, exceptions=(OSError,)
    )
    assert res == "success"
    assert attempts == 3
...
def test_retry_sync_exhausts_and_raises():
    ...
    with pytest.raises(ValueError, match="Permanent error"):
        retry_sync(
            always_fails, max_retries=3, initial_delay=0.01, backoff_factor=1.0, jitter=False, exceptions=(ValueError,)
        )
    assert attempts == 3
