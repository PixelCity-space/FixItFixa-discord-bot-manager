...
def test_update_service_update_bot_success(tmp_path):
    async def run():
...
def test_update_service_update_bot_no_changes(tmp_path):
    async def run():
        bot = BotConfig(id="b1", name="Bot One", path=str(tmp_path), cmd="python app.py")
        app_cfg = AppConfig(bots={"b1": bot})
...
def test_update_service_rollback_bot(tmp_path):
    async def run():
        bot = BotConfig(id="b1", name="Bot One", path=str(tmp_path), cmd="python app.py")
        app_cfg = AppConfig(bots={"b1": bot})
...
def test_update_service_update_non_existent_bot():
    async def run():
