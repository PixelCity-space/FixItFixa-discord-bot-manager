...
def create_mock_bot():
    ...
    app_cfg = AppConfig(
        guild_id="99999",
        bot_settings=BotSettingsConfig(language="hu")
    )
    bot = MagicMock()
...
def test_e2e_status_command_ephemeral_report_flow():
    async def run():
        ...
        telemetry_mock.get_status_snapshot.return_value = (
            {
                "cpu": 1.0, "ram": 50, "uptime": "1h", "branch": "main", "os": "Windows",
                "sys_cpu_free": 90, "sys_ram_free": 2000, "sys_disk_free": 100,
                "swap": 0, "host_uptime": "1d", "net": "1 KB/s", "has_update": False
            },
            {}
        )
